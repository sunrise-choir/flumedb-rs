...
fn main() {
    ...
    vals.push(Data{buff: vec![1,2,3], seq: 0});
    vals.push(Data{buff: vec![1,2,3], seq: 123});

    let mut log_bus = Bus::<Data>::new(10);
    let mut hash_view_rx = log_bus.add_rx();

    let (hash_view_cmd_tx, hash_view_cmd_rx): (Sender<HashViewCmd>, Receiver<HashViewCmd>) = mpsc::channel();  
        ...
        loop {
            ...
            match hash_view_cmd_rx.try_recv() {
                Ok(HashViewCmd::GetSeq(hash, request_num, sender)) => {
                    trace!("get Seq Cmd for hash {}, request num is  {}", hash, request_num);
    ...
    hash_view_cmd_tx.send(HashViewCmd::GetSeq("somehash".to_string(), 1, cb_tx)).unwrap();
